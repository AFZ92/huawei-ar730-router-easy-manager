...
check("أزرار الشريط الإنجليزية بمحاذاة يسار", all(button.property("rtl") is False for button in window.nav))
...
check("أزرار الشريط العربية بمحاذاة يمين", all(button.property("rtl") is True for button in window.nav))
window._toggle_language()
app.processEvents()
check("زر اللغة يعيد الإنجليزية LTR", window.lang == "en" and window.layoutDirection() == qt.Qt.LeftToRight)
check("أزرار الشريط تعود لمحاذاة يسار", all(button.property("rtl") is False for button in window.nav))
controller.connect("demo", 22, "admin", "anything")
